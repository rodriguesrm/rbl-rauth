...
using RBlazeLabs.Architecture.Infra.Data.MongoDb.Documents;
        ...
        where TDocument : IDocument
    {
        ...
        protected async Task<bool> IsCollectionExists()
        {
            ...
            if (collections is not null)
                await collections.ForEachAsync(document => 
                    result = result || document["name"].AsString.Equals(_collectionName));
        ...
        protected async Task<IMongoCollection<TDocument>> CreateCollectionAsync(
            bool isCapped, long? maxSize, long? maxDocuments)
        {
        ...
        protected async Task<bool> IsIndexExists(IMongoCollection<TDocument> collection, params string[] indexNames)
        {
            ...
            using (IAsyncCursor<BsonDocument> cursor = await collection.Indexes.ListAsync())
            {
                ...
                foreach (BsonDocument index in existingIndexes)
                {
                    ...
                    if (currentIndex is null) continue;
        ...
        protected async Task CreateIndexAsync(
            string indexName, bool isUnique, int? lifetime, params Expression<Func<TDocument, object>>[] fields)
        {
            ...
            if (Collection is not null)
            {
                if (!await IsIndexExists(Collection, indexName))
                {
                    ...
                    if (lifetime.HasValue)
                        indexParameters = new CreateIndexOptions 
                        { 
                            Name = indexName, 
                            Unique = isUnique, 
                            ExpireAfter = TimeSpan.FromDays(lifetime.Value) 
                        };
                    else
                    ...
                    CreateIndexModel<TDocument> indexModel = new(indexDefinition, indexParameters);
        ...
        )
        {
            ...
            if (Collection is not null)
            {
                if (!await IsIndexExists(Collection, indexName))
                {
                    ...
                    CreateIndexOptions indexParameters = new() 
                    { 
                        Name = indexName, 
                        Unique = isUnique, 
                        ExpireAfter = days 
                    ...
                    CreateIndexModel<TDocument> indexModel = 
                        new(Builders<TDocument>.IndexKeys.Ascending(field), indexParameters);
                    await Collection.Indexes.CreateOneAsync(indexModel);
