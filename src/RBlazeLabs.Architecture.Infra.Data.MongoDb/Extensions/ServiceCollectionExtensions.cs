...
using RBlazeLabs.Architecture.Infra.Data.MongoDb.Abstractions;
using RBlazeLabs.Architecture.Infra.Data.MongoDb.Creators;
using RBlazeLabs.Architecture.Infra.Data.MongoDb.Options;
using System.Reflection;

namespace RBlazeLabs.Architecture.Infra.Data.MongoDb.Extensions
{
    ...
    public static class ServiceCollectionExtensions
    {
        ...
        )
        {
            IEnumerable<TypeInfo> types = 
                assemblies
                    .SelectMany(assembly => assembly.DefinedTypes.Where(x => x.GetInterfaces().Contains(typeof(T))));
        ...
        )
        {
            ...
            services.Configure<MongoDbConnectionStrings>(options => 
                configuration.GetSection("ConnectionStrings").Bind(options));

            //MongoDB
            services.AddSingleton<MongoDatabaseProvider>();
            services.AddSingleton(s => 
                s.GetService<MongoDatabaseProvider>()?.GetDatabase()
                ?? throw new InvalidOperationException(
                    $"{nameof(MongoDatabaseProvider)} cannot to added to Service Collection"));
